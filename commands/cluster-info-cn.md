...
*   `cluster_slots_assigned`：与某个节点 (未未绑) ) 关联的插槽数。此数字应为 16, 84, 节点才能正, 工作, 这意味着每个哈希槽都应映射到一个节点。
...
*   `cluster_stats_messages_ping_sent`和`cluster_stats_messages_ping_received`：群集总线 PING (不要与客户端命令混) ) `PING`).
*   `cluster_stats_messages_pong_sent`和`cluster_stats_messages_pong_received`：PONG (回复 PIN) ) 。
