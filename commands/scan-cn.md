...
但是，因为`SCAN`具有很少的关联状态（只是游标），它具有以下缺点：
...
但是，返回的元素数是合理的，也就是说，实际上，在迭代大型集合时，SCAN 可能会以几十个元素的顺序返回最大数量的元素，或者当迭代的集合足够小，可以在内部表示为编码数据结构时，在单个调用中返回集合的所有元素（对于小集合， 哈希和排序集）。
...
*   当迭代编码为intset（仅由整数组成的小集合）或编码为ziplist的Hashes和Sorted Sets（小哈希和由小的单个值组成的集）时，通常所有元素都在第一个`SCAN`调用，而不考虑 COUNT 值。
...
为此，只需附加`MATCH <pattern>`末尾的参数`SCAN`命令（它适用于所有 SCAN 系列命令）。
...
这`type`参数是与`TYPE`命令返回。请注意一个怪癖，其中某些 Redis 类型（如 GeoHashes、HyperLogLogs、Bitmaps 和 Bitfields）可能在内部使用其他 Redis 类型（如字符串或 zset）实现，因此无法通过以下方式与相同类型的其他键区分开来。`SCAN`.例如，ZSET 和 GEOHASH：
...
`SCAN`,`SSCAN`,`HSCAN`和`ZSCAN`返回两个元素的多批量回复，其中第一个元素是表示无符号 64 位数字（游标）的字符串，第二个元素是具有元素数组的多批量。
