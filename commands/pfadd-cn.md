...
如果 HyperLogLog 估计的近似基数在执行命令后发生更改, `PFADD`返回 1, 否则返回 0。如果指定的键不存在, 则该命令会自动创建一个空的 HyperLogLog 结构 (, , 具有指定长度且具有给定编码的 Redis 字符) ) 。

要调用不带元素但只有变量名称有效的命令, 如果变量已存在, 这将导致不执行任何操作, 或者如果键不存在, 则仅创建数据结构 (在后一种情况下返回 ) ) 。
