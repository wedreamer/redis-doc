...
## 标题： “Redis 模式示例”&#xA;链接标题： “模式示例”&#xA;描述： 通过构建 Twitter 克隆了解几种 Redis 模式&#xA;体重： 20
...
键值存储提供的其他常见操作包括`DEL`，删除给定键及其关联值，SET-if-不存在（称为`SETNX`），仅当某个键尚不存在时才为该键赋值，以及`INCR`，以原子方式递增存储在给定键中的数字：
...
`LPUSH`方法*左推*，即，将元素添加到 存储在 的列表的左侧（或头部）*我的列表*.如果密钥*我的列表*不存在，它会在 PUSH 操作之前自动创建为空列表。可以想象，还有一个`RPUSH`将元素添加到列表右侧（在尾部）的操作。这对我们的Twitter克隆非常有用。可以将用户更新添加到存储在`username:updates`例如。
...
更新（也称为帖子）甚至更简单。为了在数据库中创建新帖子，我们执行如下操作：
