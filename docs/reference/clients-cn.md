...
每个客户端还受到查询缓冲区限制的约束。这是一个不可配置的硬限制，当客户端查询缓冲区（即我们用于从客户端累积命令的缓冲区）达到 1 GB 时将关闭连接，实际上只是一个极端限制，以避免在客户端或服务器软件错误的情况下服务器崩溃。
...
超时不应被视为非常精确：Redis 避免设置计时器事件或运行 O（N） 算法以检查空闲客户端，因此检查不时以增量方式执行。这意味着，当超时设置为 10 秒时，客户端连接可能会关闭，例如，如果同时连接多个客户端，则在 12 秒后。
...
从 3.2 版本开始，Redis 具有 TCP 保持警惕 （`SO_KEEPALIVE`套接字选项）默认启用，并设置为约 300 秒。此选项对于检测失效的对等体（即使它们看起来已连接也无法到达的客户端）非常有用。此外，如果客户端和服务器之间存在需要查看某些流量才能打开连接的网络设备，则该选项将防止意外的连接关闭事件。
