...
**注意：**
沙盒尝试阻止使用全局变量。
使用Lua的调试功能或其他方法（例如更改用于实现全局保护的元表以绕过沙盒）并不难。
但是，很难意外地规避保护。
...
此函数允许执行脚本在[Redis 序列化协议 （RESP）](/topics/protocol)返回的答复的版本[`redis.call()](#redis.call) and [`redis.pall（）](#redis.pcall).
它期望单个数字参数作为协议的版本。
...
然后，它选取五个随机元素 （`SRANDMEMBER`） 从十字路口和商店 （`SADD`） 它们在另一组中。
...
此函数在使用 Redis Lua 调试器]（/topics/ldb） 时触发断点。
...
*   `no-writes`：此标志表示脚本仅读取数据，但从不写入。
    ...
    2.  在只读副本上。
    3.  即使存在磁盘错误（Redis 无法持久存在，因此会拒绝写入）。
    4.  当超过内存限制时，因为它意味着脚本不会增加内存消耗（请参阅`allow-oom`下）
...
从脚本返回的 Lua 数据类型进行类型转换取决于用户选择的协议（请参阅`HELLO`命令）。
...
*   [RESP2 阵列回复](/topics/protocol#resp-arrays)-> Lua 表（可能嵌套了其他 Redis 数据类型）
...
*   Lua 数字 ->[RESP2 整数回复](/topics/protocol#resp-integers)（数字转换为整数）
*   Lua 字符串 ->[RESP 批量字符串回复](/topics/protocol#resp-bulk-strings)
*   Lua 表（索引，非关联数组）->[RESP2 阵列回复](/topics/protocol#resp-arrays)（在第一个 Lua 处截断`nil`表中遇到的值（如果有）
...
*   Lua有一个单一的数字类型，Lua数字。
    整数和浮点数之间没有区别。
    因此，我们始终将Lua数字转换为整数回复，删除数字的小数部分（如果有）。
    **如果要返回 Lua 浮点数，则应将其作为字符串返回**,
    与 Redis 本身完全相同（例如，请参阅`ZSCORE`命令）。
...
无论脚本选择的协议版本设置为使用 \[`redis.setresp()`函数] 当它调用`redis.call()`或`redis.pcall()`，用户可以选择使用 RESP3（使用`HELLO 3`命令）表示连接。
...
*   这[*字符串操作（字符串）*图书馆](https://www.lua.org/manual/5.1/manual.html#5.4)
*   这[*表操作（表）*图书馆](https://www.lua.org/manual/5.1/manual.html#5.5)
*   这[*数学函数（数学）*图书馆](https://www.lua.org/manual/5.1/manual.html#5.6)
...
*   `l/L`：有符号/无符号长
*   `T`： size_t
*   `i/In`：带大小的有符号/无符号整数*n*（默认为整型的大小）
*   `cn`：序列*n*字符（从/到字符串）;打包时，n == 0 表示
