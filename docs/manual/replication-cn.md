...
1.  当主实例和副本实例连接良好时，主实例通过向副本发送命令流来复制由于以下原因而对主端发生的数据集的影响，从而保持副本更新：客户端写入、密钥过期或逐出，任何其他操作更改了主数据集。
...
Redis 默认使用异步复制，即低延迟和高性能，是绝大多数Redis的自然复制模式用例。但是，Redis 副本会异步确认数据量他们定期与主人一起接收。所以主人不会每次都等着对于要由复制副本处理的命令，但是如果需要，它知道什么复制副本已经处理了什么命令。这允许具有可选的同步复制。

客户端可以使用以下命令请求某些数据的同步复制这`WAIT`命令。然而`WAIT`只能确保有其他 Redis 实例中指定数量的已确认副本，它不会将一组 Redis 实例转换为具有强一致性的 CP 系统：已确认在故障转移期间，写入操作仍可能丢失，具体取决于确切的配置的 Redis 持久性。但是`WAIT`丢失写入的概率故障后事件大大减少到某些难以触发的故障模式。
...
在使用 Redis 复制的设置中，强烈建议在主服务器和副本中启用了持久性。当这是不可能的，例如，由于磁盘速度非常慢而导致的延迟问题，实例应配置为**避免自动重新启动**重新启动后。

为了更好地理解为什么关闭持久性的主节点配置为自动重启是危险的，检查以下故障模式，其中数据从主服务器及其所有副本中擦除：
...
当主服务器上的密钥过期时，它将作为 DEL 命令传播。
如果主节点上存在但被删除、过期或副本上与主节点具有不同类型的密钥，则对从主服务器传播的 DEL、INCR 或 RPOP 等命令的反应将与预期不同。
传播的命令可能会在复制副本上失败或导致不同的结果。
为了最大限度地降低风险（如果您坚持使用可写副本），我们建议您遵循以下建议：

*   不要写入可写副本中也在主服务器上使用的密钥。
...
从7.0版本开始，这些用例现在都已经过时了，可以通过其他方式实现相同的操作。
例如：
...
在 4.0 版之前，可写副本无法使密钥在生存时间设置后过期。
这意味着，如果您使用`EXPIRE`或者为密钥设置最大 TTL 的其他命令，密钥将泄漏，虽然您在使用读取命令访问密钥时可能不再看到它，但您将在密钥计数中看到它，并且它仍然会使用内存。
Redis 4.0 RC3 及更高版本能够像主服务器一样使用 TTL 逐出密钥，但以大于 63 的数据库编号写入的密钥除外（但默认情况下，Redis 实例只有 16 个数据库）。
请注意，即使在大于 4.0 的版本中，使用`EXPIRE`在主服务器上可能存在的密钥上可能会导致副本和主服务器之间不一致。
...
Redis 过期允许密钥具有有限的生存时间 （TTL）。这样的功能取决于实例计算时间的能力，但 Redis 副本正确复制具有过期的密钥，即使使用 Lua 更改此类密钥脚本。

要实现这样的功能，Redis不能依靠主和复制副本具有同步时钟，因为这是一个无法解决的问题并且会导致竞争条件和数据集发散，因此Redis使用三种主要技术来复制过期的密钥能够工作：

1.  副本不会使密钥过期，而是等待主节点使密钥过期。当主密钥过期（或由于 LRU 而将其逐出）时，它会合成`DEL`命令，该命令将传输到所有副本。
2.  但是，由于主节点驱动的过期，有时副本可能仍然具有逻辑上已过期的密钥，因为主副本无法提供`DEL`及时命令。为了解决这个问题，副本使用其逻辑时钟来报告密钥不存在**仅用于读取操作**不违反数据集的一致性（因为来自主服务器的新命令将到达）。通过这种方式，副本可以避免报告仍然存在的逻辑过期密钥。实际上，使用副本进行缩放的 HTML 片段缓存将避免返回已早于所需生存时间的项目。
3.  在 Lua 脚本执行期间，不执行任何关键过期。当 Lua 脚本运行时，从概念上讲，主脚本中的时间将被冻结，因此给定的键在脚本运行的所有时间内要么存在，要么不存在。这可以防止密钥在脚本中途过期，并且需要以保证在数据集中具有相同效果的方式将相同的脚本发送到副本。

一旦复制副本被提升为主服务器，它将开始独立地使密钥过期，并且不需要其旧主节点的任何帮助。
...
要解决这两个问题，从 Redis 3.2.2 开始，可以强制
一个副本，用于向主服务器宣布任意一对 IP 和端口。
要使用的两个配置指令是：
...
这意味着密钥的逐出将由主站处理，将DEL命令发送到副本，因为主站端的键被逐出。
